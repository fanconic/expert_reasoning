...
def _extract_cli_out_name() -> str | None:
    """
    Extract an optional CLI output-name argument before Hydra parses argv.
    Accepted flags: --out-name, --out_name
    """
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument("--out-name", "--out_name", dest="out_name", type=str, default=None)
    args, remaining = parser.parse_known_args(sys.argv[1:])
    sys.argv = [sys.argv[0]] + remaining
    return args.out_name


CLI_OUT_NAME = _extract_cli_out_name()
...
def main(cfg: DictConfig):
    ...
    print("-" * 50)
    out_name = CLI_OUT_NAME or getattr(cfg, "out_name", "no_out_name_specified")
    ...
    jsonl_path = f"{cfg.model.policy_name}/eval_results_new.jsonl"

    # Check if it exists on disk
    if not os.path.exists(jsonl_path):
        jsonl_path = f"{cfg.model.policy_name}/debug.jsonl"
        if not os.path.exists(jsonl_path):
            jsonl_path = f"{cfg.model.policy_name}/eval_results.jsonl"
    ...
    output_filename = out_name if out_name.endswith(".jsonl") else f"{out_name}.jsonl"
    output_file = f"{cfg.model.policy_name}/{output_filename}"
