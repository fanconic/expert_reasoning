...
def irl_load_model_and_tokenizer_trl(config):
    ...
    use_grad_ckpt = (
        True if getattr(config.model, "use_gradient_checkpointing", True) else False
    )

    torch.manual_seed(random_state)

    # ----- Tokenizer -----
    policy_tokenizer = AutoTokenizer.from_pretrained(policy_model_name, use_fast=True)
    ...
    policy_tokenizer.model_max_length = max_seq_length
    ...
    reward_tokenizer.model_max_length = max_seq_length
    ...
    if len(policy_tokenizer) != model.get_input_embeddings().weight.shape[0]:
        model.resize_token_embeddings(len(policy_tokenizer))
