...
def main():
    ...
    formatted_data = {
        m: {a: {dataset: "-" for dataset in DATASETS} for a, _ in ALGO_ORDER}
        for m, _ in MODEL_ORDER
    }
    ...
    for model_key, model_name in MODEL_ORDER:
        ...
        for algo_key, algo_name in ALGO_ORDER:
            ...
            if algo_key == "GRPO":
                ...
                latex.append(
                    r"\arrayrulecolor{gray!60}\cdashline{1-6}\arrayrulecolor{black}"
                )
    ...
    latex.append(r"}")
    latex.append(
        r"\caption{\textbf{Pass@1 Performance (\%).} \textbf{Bold} indicates the best performance compared between SFT and our methods. Verifiable reward is provided as a reference upper bound. * symbolises an adversarial mode collapse (results grayed out). The values inside brackets indicate the 95\% confidence interval.}"
    )
