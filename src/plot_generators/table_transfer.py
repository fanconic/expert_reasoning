...
def get_delta(res):
    ...
    if not res["Random"] or not res["Reward"]:
        return None
    try:
    ...
    for i, (p_ds_key, p_ds_name) in enumerate(DATASETS):
        ...
        for rm_key, _ in RM_MODELS:
            for r_ds_key, _ in DATASETS:
                folder_name = (
                    f"transfer_{rm_key}_{METHOD_KEY}_P_{p_ds_key}_R_{r_ds_key}"
                )
                ds_path = Path(ROOT_DIR) / p_ds_key
                run_dir = resolve_run_dir(
                    ds_path,
                    folder_name,
                    required_file=f"pass_at_k_table_reranking_{NUM_GENERATIONS}.txt",
                )
                fpath = (
                    run_dir / f"pass_at_k_table_reranking_{NUM_GENERATIONS}.txt"
                    if run_dir is not None
                    else ds_path
                    / folder_name
                    / f"pass_at_k_table_reranking_{NUM_GENERATIONS}.txt"
                )
