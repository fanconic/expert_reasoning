...
def format_cell(val_str):
    ...
    try:
        clean_mean = parts[0].replace(r'\textbf{', '').replace(r'\underline{', '').replace('}', '')
...
def get_numeric_delta(rand_str, target_str):
    ...
    """
    if not rand_str or not target_str: return None
...
def main():
    ...
    for num_gen in NUM_GENERATIONS:
        ...
        for dataset in DATASETS:
            ...
            for model_key, _ in MODEL_ORDER:
                for algo_key, _ in ALGO_ORDER:
                    ...
                    fpath = os.path.join(ds_path, folder, f'pass_at_k_table_reranking_{num_gen}.txt')
        ...
        latex.append(r"& & \multicolumn{5}{c}{\textbf{\textsc{GSM8K}}} & \multicolumn{5}{c}{\textbf{\textsc{MedReason}}} & \multicolumn{5}{c}{\textbf{\textsc{MMLU-Pro}}} \\")
        ...
        latex.append(f"\\textbf{{Backbone}} & \\textbf{{Method}} & {header_metrics} & {header_metrics} & {header_metrics} \\\\")
        ...
        for model_key, model_name in MODEL_ORDER:
            ...
            for algo_key, algo_name in ALGO_ORDER:
                ...
                for dataset in DATASETS:
                    entry = data[model_key][algo_key].get(dataset, {'Random': None, 'Logprobs': None, 'Majority': None, 'Weighted_Majority': None, 'Reward': None})
                    ...
                    d_wmaj_num = get_numeric_delta(entry['Random'], entry['Weighted_Majority'])
                    
                    # Find the highest improvement (max delta) among the four
                    valid_deltas = [d for d in (d_log_num, d_maj_num, d_rew_num, d_wmaj_num) if d is not None]
                    max_d = max(valid_deltas) if valid_deltas else None
                    
                    # Format cells, explicitly passing True only to the highest value
                    d_log = format_delta_value(d_log_num, is_best=(d_log_num == max_d and max_d is not None))
                    d_maj = format_delta_value(d_maj_num, is_best=(d_maj_num == max_d and max_d is not None))
                    d_wmaj = format_delta_value(d_wmaj_num, is_best=(d_wmaj_num == max_d and max_d is not None))
                    d_rew = format_delta_value(d_rew_num, is_best=(d_rew_num == max_d and max_d is not None))
