...
def build_p1_latex_table_supervision_split(
    formatted_data, caption, label, model_order=None
):
    """
    Builds a Pass@1 LaTeX table split by supervision regime:
    (1) External verifier, (2) Demonstrations only.
    """
    active_model_order = model_order if model_order is not None else MODEL_ORDER

    latex = []
    latex.append(
        r"% Requires \usepackage{booktabs}, \usepackage{multirow}, \usepackage{arydshln}, \usepackage[table]{xcolor}"
    )
    latex.append(r"\begin{table*}[h!]")
    latex.append(r"\scriptsize")
    latex.append(r"\renewcommand{\arraystretch}{1.18}")
    latex.append(r"\setlength{\tabcolsep}{3.8pt}")
    latex.append(r"\centering")
    latex.append(r"\resizebox{\textwidth}{!}{%")
    ...
    # ---------- External verifier ----------
    latex.append(r"\rowcolor{black!8}")
    latex.append(
        rf"\multicolumn{{{total_columns}}}{{l}}{{\textbf{{External verifier}}}} \\"
    )

    grpo_cells = [
        formatted_data[model_key]["GRPO"][dataset]
        for dataset, _dataset_label in DATASET_COLUMNS
        for model_key, _model_name in active_model_order
    ]
    latex.append(rf"Outcome Reward & -- & {' & '.join(grpo_cells)} \\")
    latex.append(r"\specialrule{0.9pt}{0.15em}{0.15em}")

    # ---------- Demonstrations only ----------
    latex.append(r"\rowcolor{black!8}")
    latex.append(
        rf"\multicolumn{{{total_columns}}}{{l}}{{\textbf{{Demonstrations only}}}} \\"
    )
    ...
    latex.append(rf"SFT & -- & {' & '.join(sft_cells)} \\")
    latex.append(rf"\cdashline{{1-{total_columns}}}[0.5pt/1.8pt]")
...
def main():
    ...
    formatted_data_supervision = build_formatted_data(data, ALGO_ORDER)
    ...
    supervision_caption = (
        r"\caption{\textbf{Held-out pass@1 by supervision regime.} Methods are separated into \textit{External verifier} (Outcome Reward) and \textit{Demonstrations only} (SFT and R-AIRL variants). \textbf{Bold} indicates the best performance and \underline{underlined} the second best among demonstration-only methods. Values are reported as mean $\pm$ half-width of the 95\% confidence interval bootstrapped over the test set.}"
    )
    supervision_label = r"\label{tab:p1_results_supervision_split}"
    supervision_split_table = build_p1_latex_table_supervision_split(
        formatted_data_supervision,
        supervision_caption,
        supervision_label,
        model_order=MODEL_ORDER,
    )
    supervision_split_table.append("")
    ...
    output_path_supervision = os.path.join(
        ROOT_DIR, "results_p1_temp05_supervision_split.txt"
    )
    with open(output_path_supervision, "w") as f:
        f.write("\n".join(supervision_split_table).rstrip())

    print(f"Success! Created '{output_path}'")
    print(f"Success! Created '{output_path_variants}'")
    print(f"Success! Created '{output_path_supervision}'")
