...
def main():
    ...
    latex.append(
        r"\textbf{Gen. ($N$)} & \textbf{Pass@1} & \textbf{$\Delta$ Rerank} & \textbf{AUROC $\uparrow$} & \textbf{ECE $\downarrow$} \\"
    )
    ...
    for model_key, model_name in MODEL_ORDER:
        for algo_key, n_label in ALGO_ORDER:
            ...
            rand_val, rew_val = extract_reranking_metrics(
                folder_path / "pass_at_k_table_reranking.txt"
            )
    ...
    latex.append(r"}")
    latex.append(
        r"\caption{\textbf{Ablation on Number of Generations ($N$)} for \texttt{Llama3.2-3B} (Step-wise) on GSM8K. All metrics are in full percentages (\%). Intervals indicate 95\% confidence intervals.}"
    )
    ...
    print(
        f"Success! Ablation table with full percentages and CIs created at {output_file}"
    )
