...
def main():
    ...
    latex.append(r"\textbf{Corruption Type} & \textbf{Pass@1} & \textbf{$\Delta$ Rerank} & \textbf{AUROC $\uparrow$} & \textbf{ECE $\downarrow$} \\")
    ...
    latex.append(r"}")
    latex.append(r"\caption{\textbf{Ablation on Reasoning Corruption} for \texttt{Qwen2.5-3B} (Step-wise) on GSM8K. All metrics are in full percentages (\%). Intervals indicate 95\% confidence intervals.}")
    ...
    print(f"Success! Ablation table with full percentages and wrapped labels created at {output_file}")
