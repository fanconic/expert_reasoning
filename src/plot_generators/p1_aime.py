...
def main():
    ...
    for dataset in DATASETS:
        ...
        for model_key, _ in MODEL_ORDER:
            ...
            sft_found = False
            candidates = sorted(
                [f for f in os.listdir(ds_path) if f.startswith(model_key)]
            )
            ...
            for folder in candidates:
                if sft_found:
                    break
                fpath = os.path.join(
                    ds_path, folder, "pass_at_k_table_reward_weighted.txt"
                )
            ...
            for algo_key, _ in ALGO_ORDER:
                ...
                variant_file = os.path.join(
                    ds_path, variant_folder, "pass_at_k_table_reward_weighted.txt"
                )
    ...
    formatted_data = {
        m: {a: {dataset: ["-"] * 4 for dataset in DATASETS} for a, _ in ALGO_ORDER}
        for m, _ in MODEL_ORDER
    }
        ...
        for dataset in DATASETS:
            ...
            for k_idx in range(4):
                ...
                for algo_key, _ in ALGO_ORDER:
                    ...
                    col_results[k_idx][algo_key] = format_cell(
                        val_str, is_best, is_second
                    )
    ...
    for model_key, model_name in MODEL_ORDER:
        # === SUB-HEADER ROW ===
        latex.append(
            f"\\multicolumn{{{total_cols}}}{{l}}{{\\textbf{{{model_name}}}}} \\\\"
        )

        for algo_key, algo_name in ALGO_ORDER:
            dataset_vals = [
                formatted_data[model_key][algo_key][dataset][0]
                for dataset, _ in DATASET_COLUMNS
            ]

            # Add horizontal spacing for hierarchy
            latex.append(
                f"\\hspace{{1em}}{algo_name} & {' & '.join(dataset_vals)} \\\\"
            )
    ...
    latex.append(
        r"\caption{\textbf{Pass@1 Performance (\%).} \textbf{Bold} indicates the best performance compared between SFT and our methods.}"
    )
