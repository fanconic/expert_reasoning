...
def main():
    ...
    latex.append(
        r"\textbf{$\beta$ (LB, UB)} & \textbf{Pass@1} & \textbf{$\Delta$ Rerank} & \textbf{AUROC $\uparrow$} & \textbf{ECE $\downarrow$} \\"
    )
    ...
    latex.append(r"}")
    latex.append(
        r"\caption{\textbf{Ablation on Reward Clipping ($\beta$)} for \texttt{Llama3.2-3B} (Step-wise) on GSM8K. All metrics are shown in full percentages (\%). Intervals indicate 95\% confidence intervals.}"
    )
