#!/usr/bin/env bash
# Super runner (GPU 2) - balanced workload chunk
set -u
...
SFT_TRAIN_PARAMS=(
    training.max_steps=280
    eval.eval_steps=70
    training.gradient_accumulation_steps=10
...
IRL_TRAIN_PARAMS=(
    ...
    training.beta=0.1
    training.buffer_size=100
    training.max_steps=280
    eval.eval_steps=70
    model.policy_learning_rate=1e-5
...
IRL_LORA_FLAGS=(
    model.lora_rank=64
    model.policy_lora_rank=64
    model.reward_lora_rank=64
)
IRL_VARIANTS=(partial full sparse)
...
run_sft_stack() {
    ...
    local run_name="${model}_${dataset}_sft"
...
run_irl_scoring_on_sft_traces() {
    ...
    local run_name="${model}_${dataset}_${suffix}"
    local wb_project="neurips_airl_${dataset}"
    local sft_run_name="${model}_${dataset}_sft"
...
run_irl_variant() {
    ...
    warmup_override+=(training.reward_warmup_steps=160)
...
run_combo() {
    ...
    if [[ "${model}" == "llama8b" ]]; then
        echo "Skipping SFT for ${model}_${dataset}; starting directly with IRL."
    else
        run_sft_stack "${dataset}" "${model}"
    fi
    for suffix in "${IRL_VARIANTS[@]}"; do
        run_irl_variant "${dataset}" "${model}" "${suffix}"
    done
}

# Balanced set for GPU 2.
# Keep qwen3b/llama3b at the end as requested.
run_combo "mmlu" "llama8b"
run_combo "math" "qwen4b"
# run_combo "medicine" "qwen3b"
# run_combo "mmlu" "llama3b"
