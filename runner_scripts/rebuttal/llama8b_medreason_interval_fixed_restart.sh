#!/usr/bin/env bash
# Rebuttal restart: MedReason Llama-3.1-8B fixed-interval dense AIRL.
set -u
...
# Retake overrides. Keep reward_warmup_steps explicit so the reward model is
# warmed up from scratch before AIRL training; model.warmup_reward_dir remains
# unset, so no prior warmed-up reward checkpoint is loaded.
IRL_TRAIN_PARAMS=(
...
run_llama8b_medreason_interval_fixed() {
...
echo "MedReason Llama-3.1-8B fixed-interval restart succeeded on GPU ${GPU_NUM}."
