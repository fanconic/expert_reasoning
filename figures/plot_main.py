...
def main():
    ...
    parser.add_argument("--base", type=str, default=str(BASE), help="Base outputs directory containing the run folders")
    parser.add_argument("--ckpt", type=str, default=None, help="Override checkpoint number for all experiments")
    ...
    for exp in EXPERIMENTS:
        ckpt = args.ckpt if args.ckpt is not None else str(exp.get("ckpt", DEFAULT_CKPT))
        ...
        if missing:
            print(f"[WARNING] Skipping '{label}' — missing files: {[str(m) for m in missing]}")
            continue
        ...
        # Orchestrate plots
        try:
            run_all_plots(df_airl, df_sft, df_grpo, out_dir, num_generations=16, make_token_figs=not args.no_token_figs)
        except Exception as e:
